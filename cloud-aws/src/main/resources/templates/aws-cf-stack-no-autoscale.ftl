  ...
  <#if enableInstanceProfile && !existingRole>
    ...
    <#list instanceGroups as group>
        ...
        "SpotFleet": {
          ...
          "Properties": {
            "SpotFleetRequestConfigData": {
              "IamFleetRole": {"Fn::GetAtt" : ["S3AccessRole", "Arn"] },
              "SpotPrice": "${group.spotPrice}",
              "TargetCapacity": ${group.instanceCount},
      ...
      <#else>
        ...
        "AmbariNode${group.groupName?replace('_', '')}${nth}" : {
          ...
          "Properties" : {
            ...
            "ImageId"        : { "Ref" : "AMI" },
            <#if group.cloudSecurityId??>
            "SecurityGroupIds" : [ "${group.cloudSecurityId}" ],
            <#else>
            ...
            "Tags" : [ { "Key" : "Name", "Value" : { "Fn::Join" : ["-", [ { "Ref" : "StackName" }, "${group.groupName}"]] } },
                       { "Key" : "owner", "Value" : { "Ref" : "StackOwner" } },
                       { "Key" : "instanceGroup", "Value" : "${group.groupName}" }]
          }
        },
        </#list>
      </#if>
