...
def loadProperties() {
    node {
        checkout scm
        properties = readProperties file: "${WORKSPACE}/../../cloudbreak-build-master-pipeline/workspace/version"
...
pipeline {
    ...
    stages {
        ...
        stage('Add CB version to image catalog') {
            steps {
                build job: 'cloudbreak-imagecatalog-v2-append-pipeline',
                        parameters: [
                                [$class: 'StringParameterValue', name: 'CLOUDBREAK_VERSION', value: version],
                                [$class: 'StringParameterValue', name: 'CATALOG_TYPE', value: 'dev']
                        ],
                        propagate: true, wait: true
            }
        }
