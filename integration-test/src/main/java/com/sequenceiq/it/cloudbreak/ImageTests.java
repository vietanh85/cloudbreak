...
public class ImageTests extends AbstractTestNGSpringContextTests {
    ...
    String connectionAddress = "54.72.155.142";

    String publicAddress = connectionAddress;

    String privateAddress = "10.0.31.41";

    String hostname = "ip-10-0-31-41.eu-west-1.compute.internal";

    GatewayConfig gatewayConfig;

    Node node;

    ExitCriteriaModel exitModel;
    ...
    public ImageTests() {
        ...
        String saltSignPrivateKey = "-----BEGIN RSA PRIVATE KEY-----\n" +
                ...
                "-----END RSA PRIVATE KEY-----\n";
        String saltSignPublicKey = "ssh-rsa ";
...
//        when(rdsViewMock.getConnectionURL()).thenReturn("jdbc:postgresql://" + privateAddress + ":5432/" + AMBARI_DB);
//        when(rdsViewMock.getConnectionDriver()).thenReturn("org.postgresql.Driver");
//        when(rdsViewMock.getConnectionUserName()).thenReturn(AMBARI_USER);
//        when(rdsViewMock.getConnectionPassword()).thenReturn(AMBARI_DB_PASSWORD);
//        when(rdsViewMock.getDatabaseName()).thenReturn(AMBARI_DB);
//        when(rdsViewMock.getHost()).thenReturn(privateAddress);
//        when(rdsViewMock.getHostWithPortWithJdbc()).thenReturn("jdbc:postgresql://" + privateAddress + ":5432");
//        when(rdsViewMock.getSubprotocol()).thenReturn("postgresql");
//        when(rdsViewMock.getConnectionString()).thenReturn("jdbc:postgresql://" + privateAddress + ":5432/" + AMBARI_DB);
//        when(rdsViewMock.getPort()).thenReturn("5432");
//        when(rdsViewMock.getDatabaseType()).thenReturn("postgres");
//        when(rdsViewMock.getWithoutJDBCPrefix()).thenReturn(privateAddress + ":5432/" + AMBARI_DB);
    ...
    static class TestOrchestratorComponentRunner implements ParallelOrchestratorComponentRunner {
        ...
        private final AsyncTaskExecutor asyncTaskExecutor;
                ...
                public TestOrchestratorComponentRunner(AsyncTaskExecutor asyncTaskExecutor) {
