...
public class DefaulClusterDefinitionCache {
    ...
    private List<String> blueprints() {
        return Stream.concat(
                Stream.concat(
                        releasedBlueprints.stream().filter(StringUtils::isNoneBlank),
                        internalBlueprints.stream().filter(StringUtils::isNoneBlank)),
                releasedCMClusterDefinitions.stream().filter(StringUtils::isNoneBlank)
