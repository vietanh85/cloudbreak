...
public class RejectedThreadService {
    ...
    private Map<Long, RejectedThread> rejectedThreads = new HashMap<>();
    ...
    public void create(EvaluatorExecutor task) {
        ...
        if (data instanceof AutoscaleStackResponse) {
            ...
            rejectedThread = findById(stackId);
            if (rejectedThread == null) {
                rejectedThread = new RejectedThread();
                rejectedThread.setRejectedCount(1);
                rejectedThread.setId(stackId);
                rejectedThread.setJson(getJsonString(data));
            } else {
                rejectedThread.setRejectedCount(rejectedThread.getRejectedCount() + 1);
            }
        } else {
            rejectedThread = findById((Long) data);
            if (rejectedThread == null) {
                rejectedThread = new RejectedThread();
                rejectedThread.setRejectedCount(1);
                rejectedThread.setId((Long) data);
                rejectedThread.setJson(getJsonString(Collections.singletonMap("id", data)));
            } else {
                rejectedThread.setRejectedCount(rejectedThread.getRejectedCount() + 1);
            }
        }
